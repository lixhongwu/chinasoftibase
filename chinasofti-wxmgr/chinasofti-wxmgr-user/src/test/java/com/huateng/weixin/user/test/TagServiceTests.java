...
public class TagServiceTests {
	...
	public void createTest() {
		JsonObject result = tagService.createTag("逗比星人");
		if(result == null || result.size() == 0) {
			logger.error("响应失败！");
		} else {
			if(result.has("errcode")) {
				logger.error("服务器响应了异常：" + result.get("errmsg").toString() + ",错误码为：" + result.get("errcode").toString());
			} else if(result.has("tag")) {
				JsonObject tag = result.getAsJsonObject("tag");
	...
	public void getTest() {
		...
		if(result == null || result.size() == 0) {
			...
			if(result.has("errcode")) {
				...
				Gson gson = new Gson();
				List tags = gson.fromJson(result.get("tags"), List.class);
				for(Object tag : tags) {
